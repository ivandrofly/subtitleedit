...
namespace Nikse.SubtitleEdit.Logic.Ocr
{
    public class OcrFixEngine : IDisposable, IDoSpell
    {
        ...
        private void LoadSpellingDictionariesViaDictionaryFileName(string threeLetterIsoLanguageName, string dictionaryFileName, bool resetSkipList)
        {
            ...
            var dictionary = Utilities.DictionaryFolder + _fiveLetterWordListLanguageName;
            if (resetSkipList)
            {
        ...
        public string SpellCheckDictionaryName
        {
            get
            {
                var parts = _spellCheckDictionaryName?.Split(new[] { Path.DirectorySeparatorChar }, StringSplitOptions.RemoveEmptyEntries);
                if (parts?.Length > 0)
                {
        ...
        private string ReplaceWordsBeforeLineFixes(string text)
        {
            ...
            if (word.Length > 0) // last word
            {
                var fixedWord = _ocrFixReplaceList.FixCommonWordErrorsQuick(word.ToString());
                sb.Append(fixedWord);
            }
            return sb.ToString();
        }
        ...
        public string FixOcrErrorsViaHardcodedRules(string input, string lastLine, string lastLastLine, HashSet<string> abbreviationList)
        {
            var text = input;
            if (!Configuration.Settings.Tools.OcrFixUseHardcodedRules)
            {
            ...
            if (hasDotDot)
            {
                if (text.Length > 5 && text.StartsWith("..", StringComparison.Ordinal) && CharLookupSet.LettersAndNumbers.Contains(text[2]))
                {
                    text = "..." + text.Remove(0, 2);
                }

                if (text.Length > 7 && text.StartsWith("<i>..", StringComparison.Ordinal) && CharLookupSet.LettersAndNumbers.Contains(text[5]))
                {
                    text = "<i>..." + text.Remove(0, 5);
                }

                if (text.Length > 5 && text.StartsWith(".. ", StringComparison.Ordinal) && CharLookupSet.LettersAndNumbers.Contains(text[3]))
                {
                    text = "..." + text.Remove(0, 3);
                }

                if (text.Length > 7 && text.StartsWith("<i>.. ", StringComparison.Ordinal) && CharLookupSet.LettersAndNumbers.Contains(text[6]))
                {
            ...
            if ((lastLine == null || !lastLine.Contains('"')) && (lastLastLine == null || !lastLastLine.Contains('"')) &&
