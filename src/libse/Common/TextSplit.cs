...
namespace Nikse.SubtitleEdit.Core.Common
{
    public class TextSplit
    {
        ...
        public static List<string> SplitMulti(string input, int numberOfParts, string language)
        {
            ...
            var texts = SplitText(inputText, numberOfParts, i => GetBreakCost(inputText, i, language));
        ...
        {
            return SplitText(text, numberOfParts, null);
        }

        /// <summary>
        /// All the breaks are chosen together (see <see cref="TextPartition"/>). Cutting at the
        /// nearest space before each multiple of the average length, one part after the other,
        /// let every rounding pile up in the last part (26/30/36/49 for four parts).
        /// </summary>
        private static List<string> SplitText(string text, int numberOfParts, Func<int, double> breakCost)
        {
            if (numberOfParts < 2)
            {
            ...
            var breaks = TextPartition.Split(text, numberOfParts, 0, breakCost);
            if (breaks == null)
            {
                // fewer words than parts
                return text.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries).ToList();
            }

            return TextPartition.GetParts(text, breaks);
        }

        // Moving a break d characters away from the even split costs about 2 * d * d, so these
        // allow going 15 characters (the chunk TryForWholeSentences will move) out of the way to
        // end a part on a whole sentence, and 8 to end it on a comma.
        private const double BreakInsideSentenceCost = 2 * 15 * 15;
        private const double BreakAfterCommaCost = BreakInsideSentenceCost - 2 * 8 * 8;

        private static double GetBreakCost(string text, int spaceIndex, string language)
        {
            if (!Utilities.CanBreak(text, spaceIndex, language))
            {
                return Utilities.NoBreakCost;
            }

            var i = spaceIndex - 1;
            while (i > 0 && Array.IndexOf(QuoteChars, text[i]) >= 0)
            {
                i--;
            }

            if (EndLineChars.Contains(text[i]) && IsSentenceEndingAt(text, i))
            {
                return 0;
            }

            return Commas.Contains(text[i]) ? BreakAfterCommaCost : BreakInsideSentenceCost;
        }
