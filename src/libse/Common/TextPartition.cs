...
namespace Nikse.SubtitleEdit.Core.Common
{
    ...
    internal static class TextPartition
    {
        ...
        /// <summary>
        /// The pieces of <paramref name="text"/> between the given space indices.
        /// </summary>
        internal static List<string> GetParts(string text, int[] spaceIndices)
        {
            var list = new List<string>(spaceIndices.Length + 1);
            var start = 0;
            foreach (var index in spaceIndices)
            {
                list.Add(text.Substring(start, index - start).Trim());
                start = index + 1;
            }
